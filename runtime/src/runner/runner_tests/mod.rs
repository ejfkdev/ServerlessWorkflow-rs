...
mod workflow;
    ...
    fn bearer_protected_endpoint(expected_token: &str) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone + Send + Sync + 'static {
    ...
    fn basic_auth_protected_endpoint(expected_b64: &str) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone + Send + Sync + 'static {
        ...
        warp::path("protected")
            ...
            .map(move |auth: Option<String>| {
                match auth {
                    ...
                    _ => {
                        warp::reply::json(&serde_json::json!({"access": "denied"}))
                    }
                }
            })
    }
